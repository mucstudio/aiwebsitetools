...
export default async function QuickReferencePage() {
  ...
  const docPath = path.join(process.cwd(), 'lib', 'docs', 'QUICK_REFERENCE.md')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold">快速参考手册</h1>
          <p className="text-muted-foreground">直接可用的代码调用示例</p>
        </div>
            ...
            <ReactMarkdown
              components={{
                ...
                code: ({ inline, children }: any) =>
                  inline ? (
                    <code className="px-1.5 py-0.5 bg-gray-100 rounded text-sm font-mono text-red-600">{children}</code>
                  ) : (
