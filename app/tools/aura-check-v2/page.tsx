/**
 * 🌟 Aura Check 工具页面 - 工厂模式重构版
 *
 * 特点：
 * - 使用 useToolAction Hook 处理所有逻辑
 * - 保留原有的赛博朋克风格 UI
 * - 代码更简洁、更易维护
 */
...
export default function AuraCheckV2Page() {
  const [text, setText] = useState('')
  const [showOutput, setShowOutput] = useState(false)
  const [btnText, setBtnText] = useState('Calculate Aura')

  // 使用通用 Hook
  const { execute, result, loading, error, remaining } = useToolAction<{
    score: string
    body: string
    fullText: string
  }>('aura-check')
        ...
        <div className="max-w-2xl mx-auto relative pt-10" style={{ zIndex: 1 }}>
          ...
          <div className="text-center mb-12">
            <div className="inline-block border border-white/20 px-4 py-1 rounded-full backdrop-blur-md mb-4">
              <span className="text-xs uppercase tracking-[0.2em] text-gray-300">Vibe Analysis System v2</span>
            </div>
